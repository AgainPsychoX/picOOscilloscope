// Adapted example 'Touch_calibrate.ino' from TFT_eSPI library.
...
void loop(void) {
	...
	z = tft.getTouchRawZ();
	char buffer[32];
	sprintf(buffer, "x=%5u,y=%5u,z=%5u", x, y, z);
	// Serial.println(buffer);
	tft.fillRect(tft.width()/3, tft.height()/2, lastTextWidth, 16, TFT_BLACK);
	lastTextWidth = tft.drawString(buffer, tft.width()/3, tft.height()/2, 2);
